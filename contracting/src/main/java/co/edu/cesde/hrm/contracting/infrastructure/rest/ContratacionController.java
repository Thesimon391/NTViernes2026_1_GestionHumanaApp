...
import co.edu.cesde.hrm.contracting.application.dto.SuspenderEmpleadoCmd;
...
public class ContratacionController {
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void suspender(@PathVariable Long id, @Valid @RequestBody SuspenderEmpleadoCmd cmd) {
        contratacionServicePort.suspender(id, cmd.motivo());
    }

    @PutMapping("/empleados/{id}/reactivar")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void reactivar(@PathVariable Long id) {
        contratacionServicePort.reactivar(id);
    }
}
