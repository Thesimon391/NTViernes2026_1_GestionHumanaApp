...
public class ContratacionUseCase implements ContratacionServicePort {
    ...
    @Override
    public void suspender(Long empleadoId, String motivo) {
        if (motivo == null || motivo.isBlank()) {
            throw new UnprocessableEntityException("El motivo de suspensión es obligatorio");
        }

        Empleado empleado = persistencePort.findEmpleadoById(empleadoId)
                .orElseThrow(() -> new NotFoundException("No existe el empleado solicitado"));

        if (empleado.getEstado() != EstadoEmpleado.ACTIVO && empleado.getEstado() != EstadoEmpleado.EN_PERIODO_PRUEBA) {
            throw new UnprocessableEntityException("Solo se puede suspender a un empleado ACTIVO o EN_PERIODO_PRUEBA");
        }
    ...
    public void reactivar(Long empleadoId) {
        ...
        if (empleado.getEstado() != EstadoEmpleado.SUSPENDIDO) {
            throw new UnprocessableEntityException("Solo se puede reactivar a un empleado SUSPENDIDO");
        }

        Contrato contrato = persistencePort.findUltimoContratoByEmpleadoId(empleadoId)
                .orElseThrow(() -> new NotFoundException("No existe un contrato asociado al empleado"));

        contrato.marcarComoVencidoSiAplica(LocalDate.now());
        if (contrato.getEstado() == EstadoContrato.VENCIDO) {
            persistencePort.saveContrato(contrato);
            throw new UnprocessableEntityException("El contrato está VENCIDO. Debe renovarse primero");
        }

        if (contrato.getEstado() != EstadoContrato.VIGENTE) {
            throw new UnprocessableEntityException("Solo se puede reactivar si el contrato está VIGENTE");
        }
